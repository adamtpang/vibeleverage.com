...
One sentence: mapping the real branches for Fulcrum Capital before any build, given Adam is in a reserves-protecting position, not a reserves-building one.
...
- **Distance from other branches:** the safest and most crowded branch, closest in spirit to Fulcrum Capital's intended design.
...
- **Distance from other branches:** the branch closest to Fulcrum Capital's likely eventual shape if it ever needs to touch crypto-native rails, still gated, just gated in advance instead of per-transaction.
