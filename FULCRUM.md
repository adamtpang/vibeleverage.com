# FULCRUM.md: the four lever agents

Architecture for turning archimedes.life from a diagnosis into four agents
...
product names; using them would read as an Anthropic release, not an
archimedes.life one. The family name here is **Fulcrum**, the point
where a lever actually applies force, which is the whole thesis of this
site already ("Find the lever. Move the world."). Four agents:
...
first). Today those render as text in `cure-protocol.tsx`. A Fulcrum
agent is what happens when `moves` stops being a paragraph you read and
starts being a task list an agent executes.

```
Diagnose (exists)  →  Cure Protocol (exists, text-only)  →  Fulcrum Agent (new, executes)
lib/levers.ts          cure-protocol.tsx                     app/api/fulcrum/[lever]/route.ts
```
...
Fulcrum Media needs is what those three scripts already do, plus
...
formalizing it into `app/api/fulcrum/media/route.ts` is mostly wiring,
not new invention.

## Fulcrum Capital

**Built (2026-08-14), the draft-only slice of this spec.** `lib/fulcrum/capital-jobs.ts`
+ `app/fulcrum/capital`: given a subject and context, the local `claude` CLI runs with
...
draft to a local file, `.fulcrum/capital-drafts/<id>.md`. There is no send step and no
...
**Built (2026-08-14), the AI-labor half of this spec.** `lib/fulcrum/labor-jobs.ts` +
`app/fulcrum/labor`: a planner call decomposes a recurring task into 3-5 independent
