...
// Fulcrum Labor, the AI-labor half of the spec (the human-hiring half, job
// post and screening-criteria drafting that stops short of an actual hire,
// is not built yet, see FULCRUM.md). This is the one lever where the idea
...
// to mutate anything either, mutation is Fulcrum Code's job, already
// scoped and gated there.

const JOBS_DIR = path.join(process.cwd(), ".fulcrum", "labor-jobs");
...
export interface FulcrumLaborJob {
  id: string;
  task: string;
  phase: LaborJobPhase;
  createdAt: string;
...
function saveJob(job: FulcrumLaborJob) {
  ensureJobsDir();
  job.updatedAt = new Date().toISOString();
  writeFileSync(jobFile(job.id), JSON.stringify(job, null, 2), "utf-8");
}

export function loadJob(id: string): FulcrumLaborJob | null {
  const file = jobFile(id);
  if (!existsSync(file)) return null;
  return JSON.parse(readFileSync(file, "utf-8"));
}

export function listJobs(): FulcrumLaborJob[] {
  ensureJobsDir();
  return readdirSync(JOBS_DIR)
    .filter((f) => f.endsWith(".json"))
    .map((f) => JSON.parse(readFileSync(path.join(JOBS_DIR, f), "utf-8")) as FulcrumLaborJob)
    .sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

function appendLog(job: FulcrumLaborJob, line: string) {
...
export function startLaborJob(task: string): FulcrumLaborJob {
  const job: FulcrumLaborJob = {
    id: randomUUID(),
    task,
    phase: "queued",
...
const PLANNER_SYSTEM_PROMPT = `You are Fulcrum Labor's planner. Given a recurring task someone currently
...
const SYNTHESIS_SYSTEM_PROMPT = `You are Fulcrum Labor's synthesizer. You are given the original
...
/**
 * Same temp-file system-prompt pattern as every other Fulcrum agent, for
...
function runClaudeText(
  ...
  return new Promise((resolve, reject) => {
    const promptFile = path.join(os.tmpdir(), `fulcrum-labor-system-${randomUUID()}.txt`);
