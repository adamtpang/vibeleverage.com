...
// Fulcrum Capital: the one lever that structurally cannot close on its own.
// Every other Fulcrum agent ends in an action (a cut gets applied, a branch
...
//
// Powered by the local `claude` CLI, same as Fulcrum Media and Code, same
...
const JOBS_DIR = path.join(process.cwd(), ".fulcrum", "capital-jobs");
const DRAFTS_DIR = path.join(process.cwd(), ".fulcrum", "capital-drafts");
...
export interface FulcrumCapitalJob {
  id: string;
  subject: string;
  context: string;
...
function saveJob(job: FulcrumCapitalJob) {
  ensureDirs();
  job.updatedAt = new Date().toISOString();
  writeFileSync(jobFile(job.id), JSON.stringify(job, null, 2), "utf-8");
}

export function loadJob(id: string): FulcrumCapitalJob | null {
  const file = jobFile(id);
  if (!existsSync(file)) return null;
  return JSON.parse(readFileSync(file, "utf-8"));
}

export function listJobs(): FulcrumCapitalJob[] {
  ensureDirs();
  return readdirSync(JOBS_DIR)
    .filter((f) => f.endsWith(".json"))
    .map((f) => JSON.parse(readFileSync(path.join(JOBS_DIR, f), "utf-8")) as FulcrumCapitalJob)
    .sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

function appendLog(job: FulcrumCapitalJob, line: string) {
  job.log.push(`[${new Date().toISOString()}] ${line}`);
  saveJob(job);
}

/** Starts a job and returns immediately. Research continues in the background. */
export function startCapitalJob(subject: string, context: string): FulcrumCapitalJob {
  const job: FulcrumCapitalJob = {
    id: randomUUID(),
    subject,
    context,
    phase: "queued",
...
const CAPITAL_SYSTEM_PROMPT = `You are Fulcrum Capital, a draft-only pricing and invoicing research
 ...
 */
export async function saveCapitalDraft(id: string, draftDocument: string): Promise<FulcrumCapitalJob> {
...
/**
 * Same temp-file system-prompt pattern as Fulcrum Media and Code, for the
...
function runClaudeCapital(prompt: string, onLine?: (line: string) => void): Promise<string> {
  return new Promise((resolve, reject) => {
    const promptFile = path.join(os.tmpdir(), `fulcrum-capital-system-${randomUUID()}.txt`);
