...
// Vibe Code: run it, read the error, fix it, run it again, the same
...
// metered ANTHROPIC_API_KEY, same pattern as Vibe Media.
//
...
const JOBS_DIR = path.join(process.cwd(), ".vibe", "code-jobs");

export type CodeJobPhase = "queued" | "working" | "ready_for_review" | "merged" | "error";

export interface VibeCodeJob {
  id: string;
  repoPath: string;
  task: string;
  branch: string;
...
function saveJob(job: VibeCodeJob) {
  ensureJobsDir();
  job.updatedAt = new Date().toISOString();
  writeFileSync(jobFile(job.id), JSON.stringify(job, null, 2), "utf-8");
}
export function loadJob(id: string): VibeCodeJob | null {
  const file = jobFile(id);
  if (!existsSync(file)) return null;
  return JSON.parse(readFileSync(file, "utf-8"));
}
export function listJobs(): VibeCodeJob[] {
  ensureJobsDir();
  return readdirSync(JOBS_DIR)
    .filter((f) => f.endsWith(".json"))
    .map((f) => JSON.parse(readFileSync(path.join(JOBS_DIR, f), "utf-8")) as VibeCodeJob)
    .sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}
function appendLog(job: VibeCodeJob, line: string) {
...
async function runGitWithMessage(
  ...
  const msgFile = path.join(os.tmpdir(), `vibe-code-msg-${randomUUID()}.txt`);
...
export function startCodeJob(repoPath: string, task: string): VibeCodeJob {
  if (!existsSync(path.join(repoPath, ".git"))) {
    throw new Error(`${repoPath} is not a git repo (no .git found). Vibe Code only works inside git.`);
  }

  const job: VibeCodeJob = {
    id: randomUUID(),
    repoPath,
    task,
    branch: `vibe/code-${randomUUID().slice(0, 8)}`,
    baseBranch: "",
    phase: "queued",
...
const CODE_SYSTEM_PROMPT = `You are Vibe Code, a scoped coding agent. You have been given one task
...
async function processCodeJob(id: string) {
  ...
  if (status.trim().length > 0) {
    throw new Error(
      `${job.repoPath} has uncommitted changes. Vibe Code refuses to start on a dirty tree, commit or stash first.`
    );
  }
  ...
  if (afterStatus.trim().length > 0) {
    ...
    await runGitWithMessage(["commit"], `Vibe Code: ${job.task}`, job.repoPath);
  }
...
export async function mergeCodeJob(id: string): Promise<VibeCodeJob> {
...
function runClaudeCoding(cwd: string, task: string, onLine?: (line: string) => void): Promise<void> {
  return new Promise((resolve, reject) => {
    const promptFile = path.join(os.tmpdir(), `vibe-code-system-${randomUUID()}.txt`);
