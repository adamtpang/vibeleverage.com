...
// Vibe Media orchestrates the local podcast-pipeline scripts
...
const PIPELINE_DIR =
  process.env.VIBE_PIPELINE_DIR ||
  process.env.FULCRUM_PIPELINE_DIR ||
  path.join(process.cwd(), "..", "tools", "podcast-pipeline");
const PYTHON = process.env.VIBE_PYTHON || process.env.FULCRUM_PYTHON || "python";
...
function saveJob(job: VibeMediaJob) {
  ensureJobsDir();
  job.updatedAt = new Date().toISOString();
  writeFileSync(jobFile(job.id), JSON.stringify(job, null, 2), "utf-8");
}

export function loadJob(id: string): VibeMediaJob | null {
  const file = jobFile(id);
  if (!existsSync(file)) return null;
  return JSON.parse(readFileSync(file, "utf-8"));
}

export function listJobs(): VibeMediaJob[] {
  ensureJobsDir();
  return readdirSync(JOBS_DIR)
    .filter((f) => f.endsWith(".json"))
    .map((f) => JSON.parse(readFileSync(path.join(JOBS_DIR, f), "utf-8")) as VibeMediaJob)
    .sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

function appendLog(job: VibeMediaJob, line: string) {
...
export function startMediaJob(sourcePath: string): VibeMediaJob {
...
function runClaude(prompt: string, systemPrompt: string, onLine?: (line: string) => void): Promise<string> {
  return new Promise((resolve, reject) => {
    const promptFile = path.join(os.tmpdir(), `vibe-media-system-${randomUUID()}.txt`);
